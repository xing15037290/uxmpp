...
#define THIS_FILE "SocketConnection.cpp"
                                    ...
                                    socklen_t& saddr_len)
{
    ...
    if (peer_addr.type == AddrType::ipv4) {
        ...
        msg_timer.set_callback ([this](Timer& timer){
    ...
    if (get_fd() == -1) {
        ...
        msg_timer.set_callback ([errnum, this](Timer& timer){
                // Notify the connection result.
                if (connected_cb)
...
void SocketConnection::enable_tls (const TlsConfig& tls_cfg)
{
    ...
    if (!result) {
        ...
        msg_timer.set_callback ([tls_error, this](Timer& timer){
                // Notify the connection result.
                if (tls_connected_cb)
